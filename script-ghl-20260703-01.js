(function () {
  ...
  function cleanupGhlDuplicateEmbeds() {
    ...
    const latest = styleLinks.find((link) => link.href.includes("styles-ghl-20260703-01.css"))
      || styleLinks.find((link) => link.href.includes("ghl-20260703-01"))
      || styleLinks.find((link) => link.href.includes("styles-ghl-20260702-03.css"))
      || styleLinks.find((link) => link.href.includes("ghl-20260702-03"))
      || styleLinks.find((link) => link.href.includes("styles-ghl-20260702-02.css"))
      || styleLinks.find((link) => link.href.includes("ghl-20260702-02"))
      || styleLinks.find((link) => link.href.includes("styles-ghl-20260702-01.css"))
      || styleLinks.find((link) => link.href.includes("ghl-20260702-01"))
      || styleLinks.find((link) => link.href.includes("styles-ghl-20260701-04.css"))
      || styleLinks.find((link) => link.href.includes("ghl-20260701-04"))
      || styleLinks[0];
  ...
  function normalizeGhlFormFrame() {
    ...
    const frameHeight = mobile ? "690px" : "720px";
    iframe.style.height = frameHeight;
    iframe.style.minHeight = frameHeight;
    iframe.style.overflow = "hidden";
    iframe.setAttribute("data-height", mobile ? "690" : "720");
    iframe.setAttribute("scrolling", "no");
  }

  normalizeGhlFormFrame();
  window.addEventListener("load", normalizeGhlFormFrame, { once: true });
  window.addEventListener("resize", normalizeGhlFormFrame);
  window.addEventListener("message", normalizeGhlFormFrame);
  window.setTimeout(normalizeGhlFormFrame, 400);
  window.setTimeout(normalizeGhlFormFrame, 1200);
  window.setTimeout(normalizeGhlFormFrame, 2400);
  window.setTimeout(normalizeGhlFormFrame, 4200);

  let ghlNormalizeRuns = 0;
  const ghlNormalizeTimer = window.setInterval(() => {
    normalizeGhlFormFrame();
    ghlNormalizeRuns += 1;
    if (ghlNormalizeRuns >= 16) {
      window.clearInterval(ghlNormalizeTimer);
    }
  }, 500);

  window.addEventListener("load", () => {
    const iframe = document.getElementById("inline-UWryoKQjx8R0obwGyzes");
    if (iframe) iframe.addEventListener("load", normalizeGhlFormFrame);
  }, { once: true });
  ...
  if (revealItems.length) {
    ...
    window.setTimeout(revealFallback, 2200);
    window.addEventListener("load", () => window.setTimeout(revealFallback, 900), { once: true });
  }

  function warmDeferredImages() {
    document.querySelectorAll(".gallery-section img[loading='lazy'], .reviews-section img[loading='lazy'], .coque-section img[loading='lazy'], .why-section img[loading='lazy']").forEach((img) => {
      img.loading = "eager";
      const src = img.currentSrc || img.src;
      if (!src || img.dataset.warmed === "true") return;
      img.dataset.warmed = "true";
      const preload = new Image();
      preload.src = src;
    });
  }
  window.addEventListener("load", () => window.setTimeout(warmDeferredImages, 700), { once: true });
  window.setTimeout(warmDeferredImages, 1800);
